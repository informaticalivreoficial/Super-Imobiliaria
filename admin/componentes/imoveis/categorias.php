...
<div class="row">
    ...
    <div class="col-sm-6">
        <a href="painel.php?exe=imoveis/categoria-create<?= $varPage;?>" title="Cadastrar Categoria" class="btn btn-default btn-lg" style="float:right;">Cadastrar Categoria</a>
    </div>
</div>
</div>
...
<?php 
  ...
  if($delCat):
    ...
    $excluir->ExeDelete('cat_imoveis', $delCat);
    RMErro($excluir->getError()[0], $excluir->getError()[1]); 
  endif;
  
  $posti = 0;  
  $Pager = new Pager('painel.php?exe=imoveis/categorias&page=');
  $Pager->ExePager($getPage, 10);  
  $readCat = new Read;
  $readCat->ExeRead("cat_imoveis","WHERE id_pai IS NULL AND tipo = 'imovel' ORDER BY nome ASC LIMIT :limit OFFSET :offset","limit={$Pager->getLimit()}&offset={$Pager->getOffset()}");
...
<td><a class="btn btn-info" href="painel.php?exe=imoveis/categoria-edit&catid=<?= $id;?><?= $varPage;?>" title="Editar Categoria <?= $nome;?>"><i class="fa fa-pencil"></i></a>
<a title="Criar Sub-Categoria" class="btn btn-success" href="painel.php?exe=imoveis/categoria-create&catid=<?= $id;?><?= $varPage;?>"><i class="icon-file icon-white"></i>Criar sub-categoria</a>
	...
	<div class="modal-dialog">
		<div class="modal-content">				
    		...
    		<div class="modal-footer">
                ...
                <a href="painel.php?exe=imoveis/categorias&delcat=<?= $id;?><?= $varPage;?>">
...
<?php
  $readSubCat = new Read;
  $readSubCat->ExeRead("cat_imoveis","WHERE id_pai = :idpai AND tipo = 'imovel' ORDER BY nome ASC","idpai={$id}");
...
?>  
    ...
    <td><a class="btn btn-info btn-sm" href="painel.php?exe=imoveis/categoria-edit&catid=<?= $id;?>&catsubid=<?= $catSub['id'];?><?= $varPage;?>" title="Editar Sub-Categoria <?= $catSub['nome'];?>"><i class="fa fa-pencil"></i></a>   
...
<div class="modal fade" id="1<?= $catSub['id'];?>">
	<div class="modal-dialog">
		<div class="modal-content">				
    		...
    		<div class="modal-footer">
    			<button type="button" class="btn btn-default" data-dismiss="modal">Fechar</button>
                <a href="painel.php?exe=imoveis/categorias&delcat=<?= $catSub['id'];?><?= $varPage;?>">
    			<button type="button" class="btn btn-info">Confirmar</button>
                </a>
    		</div>
	 </div>
...
<?php
// Chama o Paginator    
$Pager->ExePaginator("cat_imoveis","WHERE id_pai IS NULL AND tipo = 'imovel' ORDER BY nome ASC");       	
?>
<div class="row-fluid">
    <div class="span6">
        <?php
            $readPostsCount = new Read;
            $readPostsCount->ExeRead("cat_imoveis","WHERE id_pai IS NULL AND tipo = 'imovel' ORDER BY nome ASC");
	        if($readPostsCount->getResult()):              
         ?> 
         <div class="dataTables_info" id="dynamic-table_info">Exibindo <?= $Pager->getPage();?> de <?= $Pager->getTotal("cat_imoveis");?> de <?= $readPostsCount->getRowCount();?> Categoria(s)</div>
         <?php     
            endif;
          ?>
