...
<div class="row">
    ...
    <div class="col-sm-6">
        <a href="painel.php?exe=imoveis/categorias<?= $varPage;?>" title="Voltar e Listar categorias" class="btn btn-primary btn-lg right" style="float:right;"><i class="fa fa-mail-reply"></i> Voltar e Listar categorias</a>	
    </div>
</div>
</div>
...
<?php
    ...
    $cadastra->ExeUpdate('cat_imoveis', $catid, $post);
    
    RMErro($cadastra->getError()[0], $cadastra->getError()[1]);
    
    else:
        $read = new Read;
        $read->ExeRead("cat_imoveis", "WHERE id = :id", "id={$catid}");
        if (!$read->getResult()):
            header('Location: painel.php?exe=imoveis/categorias&empty=true');
        else:
...
<?php
    ...
    require('./models/AdminCategorias.class.php');
    $cadastra = new AdminCategorias;
    $cadastra->ExeUpdate('cat_imoveis', $catsubid, $post);
    
    RMErro($cadastra->getError()[0], $cadastra->getError()[1]);
    
    else:
        $read = new Read;
        $read->ExeRead("cat_imoveis", "WHERE id = :id", "id={$catsubid}");
        if (!$read->getResult()):
            header('Location: painel.php?exe=imoveis/categorias&empty=true');
        else:
            $post = $read->getResult()[0];
