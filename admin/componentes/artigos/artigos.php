...
<div class="row">
    ...
    <div class="col-sm-6">
        <a href="painel.php?exe=artigos/artigos-create<?= $varPage;?>" title="Cadastrar Artigo" class="btn btn-default btn-lg" style="float:right;">Cadastrar Artigo</a>
    </div>
</div>
</div>
...
<?php
    ...
    if ($empty):
        RMErro("Oppsss: Você tentou editar um artigo que não existe no sistema!", RM_INFOR);
    endif;
    ...
    if($action):
        ...
        switch($action):
            case 'delete':
                ...
                RMErro("O artigo foi excluído com sucesso no sistema!", RM_ACCEPT);
                //RMErro($postUpdate->getError()[0], $postUpdate->getError()[1]);
            break;
            
            case 'Rascunho':
                $postUpdate->ExeStatus($postAction, '0');
                RMErro("O status do artigo foi atualizado para <b>Rascunho</b>. O Artigo agora é um rascunho!", RM_ALERT);                
            break;
            
            case 'Publicar':
                $postUpdate->ExeStatus($postAction, '1');
                RMErro("O status do artigo foi atualizado para <b>Publicado</b>. O Artigo publicada!", RM_ACCEPT);                
            break;
    ...
    $posti = 0;
    $getPage = filter_input(INPUT_GET, 'page', FILTER_VALIDATE_INT);
    $Pager = new Pager('painel.php?exe=artigos/artigos&page=');
    $Pager->ExePager($getPage, 25);
    
    $readPosts = new Read;
    $readPosts->ExeRead("posts","WHERE tipo = 'artigo' ORDER BY status ASC, data DESC LIMIT :limit OFFSET :offset","limit={$Pager->getLimit()}&offset={$Pager->getOffset()}");
...
<td>
<a href="painel.php?exe=artigos/artigos&post=<?= $id;?>&action=<?= $stSta;?><?= $varPage;?>" title="<?= $stSta;?>"><?= $statusPost;?></a>        
<a style="color: #333 !important;" class="btn btn-default btn-sm" title="Editar" href="painel.php?exe=artigos/artigos-edit&postid=<?= $id;?><?= $varPage;?>"><i class="fa fa-pencil"></i></a>
<a title="Visualizar" href="../blog/artigo/<?= $url;?>" target="_blank" class="btn btn-info btn-sm"><i class="fa fa-search"></i></a>        
...
echo '<div class="modal fade" id="1'.$id.'">
		<div class="modal-dialog">
			<div class="modal-content">				
    			...
    			<p>
    				<small>Você tem certeza que deseja excluir este Artigo?<br />
        		...
        		<div class="modal-footer">
                    ...
                    <a href="painel.php?exe=artigos/artigos&post='.$id.'&action=delete'.$varPage.'">
...
// Chama o Paginator    
$Pager->ExePaginator("posts","WHERE tipo = 'artigo' ORDER BY status ASC");       	
?>
<div class="row-fluid">
    <div class="span6">
        <?php
            $readPostsCount = new Read;
            $readPostsCount->ExeRead("posts","WHERE tipo = 'artigo' ORDER BY status ASC");
	        if($readPostsCount->getResult()):              
         ?> 
         <div class="dataTables_info" id="dynamic-table_info">Exibindo <?= $Pager->getPage();?> de <?= $Pager->getTotal("posts");?> de <?= $readPostsCount->getRowCount();?> Artigo(s)</div>
         <?php     
            endif;
          ?>
