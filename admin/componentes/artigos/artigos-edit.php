...
<?php
    ...
    RMErro("O Artigo <b>{$post['titulo']}</b> foi atualizado com sucesso no sistema!", RM_ACCEPT);
    
    if (!empty($_FILES['gallery_covers']['tmp_name'])):
        $sendGallery = new AdminPost;
        $sendGallery->gbSend($_FILES['gallery_covers'], $postId, 'artigos/imagens');        
    endif;
        ...
        if(!$read->getResult()):
            header('Location: painel.php?exe=artigos/artigos&artigos-create=true');
        else:
    ...
    if ($checkCreate && empty($cadastra)):
        RMErro("O Artigo <b>{$post['titulo']}</b> foi cadastrado com sucesso no sistema!", RM_ACCEPT);
    endif;
?>
...
<div class="row" id="gbfoco">
    ...
    ?>
        <div style="text-align: center;margin-bottom: 10px;" class="col-lg-3">		
            ...
            <br />
            <a href="painel.php?exe=artigos/artigos-edit&postid=<?= $postId; ?>&gbdel=<?= $gb['id']; ?>#gbfoco" title="Exluir imagem"><button class="btn btn-danger btn-xs" style="margin-top: 5px;" type="button">Excluir</button></a>
