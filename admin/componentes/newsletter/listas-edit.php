...
<?php
    ...
    $cadastra->ExeUpdateLista($postId, $post);
    
    RMErro($cadastra->getError()[0], $cadastra->getError()[1]);
    
    else: 
        $read = new Read;
        $read->ExeRead("newsletter_cat","WHERE id = :id","id={$postId}");
        if(!$read->getResult()):
            header('Location: painel.php?exe=newsletter/listas&create=true');
        else:
    ...
    if ($checkCreate && empty($cadastra)):
        RMErro("A Lista <b>{$post['titulo']}</b> foi cadastrada com sucesso no sistema!", RM_ACCEPT);
    endif;
?> 
