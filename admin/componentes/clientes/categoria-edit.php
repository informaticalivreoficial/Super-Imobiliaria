...
<div class="row">
    ...
    <div class="col-sm-6">
        <a href="painel.php?exe=clientes/categorias<?= $varPage;?>" title="Voltar e Listar categorias" class="btn btn-primary btn-lg right" style="float:right;"><i class="fa fa-mail-reply"></i> Voltar e Listar categorias</a>	
    </div>
</div>
</div>
...
<?php
    ...
    $cadastra->ExeUpdate('cat_clientes', $catid, $post);
    
    RMErro($cadastra->getError()[0], $cadastra->getError()[1]);
    
    else:
        $read = new Read;
        $read->ExeRead("cat_clientes", "WHERE id = :id", "id={$catid}");
        if (!$read->getResult()):
            header('Location: painel.php?exe=clientes/categorias&empty=true');
        else:
            ...
            <div class="panel-body">
                <div class="row">                               
                    <div class="col-md-6 form-group">
                        ...
                        <input type="hidden" name="data" value="<?php if(!empty($post['data'])): echo $post['data']; else: echo date('d/m/Y'); endif;?>"/>
                        <input type="text" class="form-control input-lg" name="nome" value="<?php if(isset($post['nome'])) echo $post['nome'];?>" />
                    </div>
                    <div class="col-md-3 form-group">
                        <label><strong>Exibir no site?</strong></label>
...
<?php
    ...
    require('./models/AdminCategorias.class.php');
    $cadastra = new AdminCategorias;
    $cadastra->ExeUpdate('cat_clientes', $catsubid, $post);
    
    RMErro($cadastra->getError()[0], $cadastra->getError()[1]);
    
    else:
        $read = new Read;
        $read->ExeRead("cat_clientes", "WHERE id = :id", "id={$catsubid}");
        if (!$read->getResult()):
            header('Location: painel.php?exe=clientes/categorias&empty=true');
        else:
            $post = $read->getResult()[0];
    ...
    <div class="col-md-12">
        ...
        <div class="panel"> 
            <div class="panel-body">
                <div class="row">                               
                    <div class="col-md-6 form-group">
                        <label><strong>Nome da Sub-Categoria</strong></label>
